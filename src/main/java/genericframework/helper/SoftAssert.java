package genericframework.helper;
...
import genericframework.abstracts.GenericBaseClass;
...
public class SoftAssert extends org.testng.asserts.SoftAssert {
	...
	private final GenericBaseClass genericBaseClass;
	private final WebDriver driver;

	/**
	 * SoftAssert constructor
	 * 
	 * @param genericBaseClass {@link GenericBaseClass GenericBaseClass}
	 *                    instance
	 */
	public SoftAssert(GenericBaseClass genericBaseClass) {
		this.genericBaseClass = genericBaseClass;
		this.driver = genericBaseClass.getBrowserFunctions().getWebDriver();
	}
...
//			}
	...
	public void addScreenshotInAllureReport(String message, Status status) throws Exception {
		...
		if (currentUrl.toLowerCase().contains("lightning")) {
			GenericCommonVariables.captureFullPageOnSoftAssert = false;
		}

		if(GenericCommonVariables.captureFullPageOnSoftAssert){
			screenshotPath = genericBaseClass.getCommonFunctions().captureFullPageScreenShot(driver, "_sa_failed");
		}else{
			screenshotPath = genericBaseClass.getCommonFunctions().captureScreenShot(driver, "_sa_failed");
		}

//		File screenshot = null;
//		String screenshotPath = genericBaseClass.getScreenshotPath() + File.separatorChar
