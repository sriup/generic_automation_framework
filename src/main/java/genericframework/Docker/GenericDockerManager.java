package genericframework.Docker;
...
public class GenericDockerManager {
	private static final String DOCKER_LOG_FILE = "docker_logs.txt";
	private static final String DOCKER_START_FILE_NAME = "startDockerSeleniumHub" + (GenericCommonVariables.osName.equalsIgnoreCase("linux")?".sh":".bat");
//	private static final String DOCKER_STOP_FILE_NAME = "stopDockerSeleniumHub.bat";
	private static final String DOCKER_STOP_FILE_NAME = "cleanup" + (GenericCommonVariables.osName.equalsIgnoreCase("linux")?".sh":".bat");
	private static final Path DOCKER_LOG_PATH = Path.of(GenericCommonVariables.DOCKER_FOLDER_PATH + File.separatorChar + DOCKER_LOG_FILE);
	...
	public void dockerOperation(String operation) throws Exception {
		...
		switch (operation.toUpperCase()) {
			...
			case "START":
				...
				dir = new File(GenericCommonVariables.DOCKER_FOLDER_PATH);
				pb.directory(dir);
				p = pb.start();
				GenericCommonVariables.docker_composer_triggered = Files.readString(Paths.get(GenericCommonVariables.DOCKER_FOLDER_PATH + File.separatorChar + "status.txt")).trim().contains("true");
