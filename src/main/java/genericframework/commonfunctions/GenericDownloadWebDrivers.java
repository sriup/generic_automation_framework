...
import genericframework.utilities.ZipUtil;
...
public class GenericDownloadWebDrivers {
	private static GenericApiMethods genericApiMethods;
	private static DateTimeUtil dateTimeUtil;
	private static ZipUtil zipUtil;
	private static FolderFileUtil folderFileUtil;
	private static final LogAccess logAccess = new LogAccess("GenericDownloadWebDrivers", LogVerboseEnums.ALL);
...
//				downloadUrl = "https://edgedl.me.gvt1.com/edgedl/chrome/chrome-for-testing/" + latestChromeVersion + "/win64/chromedriver-win64.zip";
	...
	private static String getEdgeVersion() {
		...
		Response response = genericApiMethods.sendRequest("get", uri);
		// response code
		GenericDownloadWebDrivers.logAccess.getLogger().debug("RESPONSE CODE: " + response.getStatusCode());
...
//		String uri = "https://chromedriver.storage.googleapis.com/LATEST_RELEASE";
	...
	private static void downloadWebDriver(String browserName, String laterVersion, String url, String destinationFolder)
		...
		try {
			...
			GenericDownloadWebDrivers.logAccess.getLogger()
		...
		if (!currentVersion.equals(laterVersion)) {
			...
			GenericDownloadWebDrivers.logAccess.getLogger().debug("Latest webdriver is not available for " + browserName
					+ " on the local machine. Downloading latest version # " + laterVersion);
			String originalZipPath = destinationFilePath + dateTimeUtil.getCurrentDateTime(GenericCommonVariables.TIME_FORMATS[7]) + ".zip";
			GenericDownloadWebDrivers.logAccess.getLogger().debug("Downing the driver zip and storing as " + originalZipPath);
			...
			GenericDownloadWebDrivers.logAccess.getLogger().debug("Extracting the driver to " + destinationFilePath);
