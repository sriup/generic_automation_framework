...
import genericframework.utilities.JsonUtil;
 ...
 */
public class GenericBrowserFunctions {

	/**
	 * Instantiates a new {@link GenericBrowserFunctions} object with log access.
	 *
	 * @param logAccess the instance of {@link LogAccess}
	 */
	public GenericBrowserFunctions(LogAccess logAccess) {
	...
	public void navigate(String url) {
		...
		this.driver.get(url);
		GenericCommonVariables.navigatedURLs.put(url, url);
	}
	...
	public WebDriver launch(String browserName, String downloadPath, String testCaseName) throws Exception {
		...
		switch (GenericCommonVariables.EXEC_PLATFORM.toLowerCase()) {
			case "remote":
			case "docker":
		...
		if (getWebDriver() != null) {
			...
			GenericCommonVariables.launchedBrowsers.put(caps.getBrowserName(), caps.getBrowserVersion());
		}
		return getWebDriver();
	...
	private WebDriver launchRemoteDriver(String browserName, MutableCapabilities driverCaps) throws Exception {
		RemoteWebDriver remoteDriver = new RemoteWebDriver(new URL(GenericCommonVariables.HOST_ADDRESS + "/wd/hub"), driverCaps);
	...
	private FirefoxOptions setFirefoxOptions() throws IOException {
		...
		if(GenericCommonVariables.EXEC_PLATFORM.equalsIgnoreCase("local")){
		...
		if (GenericCommonVariables.EXEC_PLATFORM.equalsIgnoreCase("local")){
				...
				GenericDownloadWebDrivers.downloadDriver(BrowserEnums.Firefox);
	...
	private ChromeOptions setChromeOptions() throws IOException {
		...
		if (GenericCommonVariables.EXEC_PLATFORM.equalsIgnoreCase("local")) {
			GenericDownloadWebDrivers.downloadDriver(BrowserEnums.Chrome);
	...
	private EdgeOptions setEdgeOptions() throws IOException {
		...
		if (GenericCommonVariables.EXEC_PLATFORM.equalsIgnoreCase("local")) {
			...
			GenericDownloadWebDrivers.downloadDriver(BrowserEnums.Edge);
