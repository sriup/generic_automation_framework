...
public class JiraConnector {
    ...
    private static synchronized Map<String, String> getIssuesStatus(String defectIds) {
        ...
        if (!defectIds.isEmpty()) {
            ...
            GenericApiMethods genericApiMethods = new GenericApiMethods(defectsLogAccess);
            ...
            try {
                // capture response
                Response response = genericApiMethods.sendRequest("get", endPointURI, headers);
                ...
                if(response.getStatusCode() == 200) {
                    ...
                    for (String issue : defectIds.split(",")) {
                        ...
                        String status = genericApiMethods.getValue(response, "issues.findAll{issues -> issues.key=='" + issue.trim() + "'}.fields.status.name[0]");
