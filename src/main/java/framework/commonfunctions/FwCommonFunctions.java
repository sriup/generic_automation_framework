...
import framework.constants.FwCommonVariables;
 ...
 * class.
 */
public class FwCommonFunctions {
	 ...
	 */
	public FwCommonFunctions(String screenShotsPath, LogAccess logAccess) {
	...
	public String getScreenShotTime() throws Exception {
		return dateTimeUtil.getCurrentDateTime(FwCommonVariables.TIME_FORMATS[7]);
	}
	...
	public WebElement waitForElement(WebDriver driver, WebElement element, ExpectedConditionsEnums expectedCondition) {
		...
		return waitUntilElement(driver, element, expectedCondition, FwCommonVariables.MAX_TIMEOUT);
	}
	...
	public WebElement waitForElement(WebDriver driver, By byLocator, ExpectedConditionsEnums expectedCondition) {
		...
		return waitUntilElement(driver, byLocator, expectedCondition, FwCommonVariables.MAX_TIMEOUT);
	}
	...
	public boolean isElementPresent(WebDriver driver, WebElement element) {
		...
		try {
			waitForElement(driver, element, ExpectedConditionsEnums.VISIBLE, FwCommonVariables.MED_TIMEOUT);
	...
	public boolean isElementDisplayed(WebDriver driver, By byLocator) {
		...
		try {
			waitForElement(driver, byLocator, ExpectedConditionsEnums.VISIBLE, FwCommonVariables.MED_TIMEOUT);
			// return true if the element is visible
			return true;
		} catch (Exception e) {
			// return false if the element is not
	...
	private WebElement waitUntilElement(WebDriver driver, WebElement element, ExpectedConditionsEnums expectedCondition,
		...
		case VISIBLE:
			return (new WebDriverWait(driver, FwCommonVariables.MAX_TIMEOUT))
					.until(ExpectedConditions.visibilityOf(element));
		case PRESENCE:
			return (new WebDriverWait(driver, FwCommonVariables.MAX_TIMEOUT)).until((WebDriver lDriver) -> element);
		default:
