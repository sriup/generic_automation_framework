...
public class DownloadWebDrivers {
	...
	public static synchronized void downloadDriver(String browserName) {
		...
		case "chrome":
			...
			downloadUrl = "https://chromedriver.storage.googleapis.com/" + latestChromeVersion  +"/chromedriver_win32.zip";
			downloadWebDriver(browserName.toLowerCase(), latestChromeVersion, downloadUrl, "drivers" + File.separatorChar + "Chrome");
			break;
		case "firefox":
			String latestGeckoVersion = getGeckoDriverVersion() ;
			downloadUrl = "https://github.com/mozilla/geckodriver/releases/download/" + latestGeckoVersion + "/geckodriver-" + latestGeckoVersion + "-win64.zip";
			downloadWebDriver(browserName.toLowerCase(), latestGeckoVersion, downloadUrl, "drivers" + File.separatorChar + "FireFox");
			break;
		case "ie":
	...
	private static void downloadWebDriver(String browserName, String laterVersion, String url, String destinationFolder) throws Exception {
		// get json data
		String currentVersion = "";
		String driversFolderPath = System.getProperty("user.dir") + File.separatorChar +  "drivers" ;
		DocumentContext jPathDocCon = null;
		try {
			jPathDocCon = com.jayway.jsonpath.JsonPath.parse(new File(driversFolderPath + File.separatorChar + "DriversInfo.json"));
			currentVersion = jPathDocCon.read(browserName + ".version").toString();
			DownloadWebDrivers.logAccess.getLogger().debug("Current available driver version for  " + browserName + " is " + currentVersion);
			 
		} catch(Exception e) {
			DownloadWebDrivers.logAccess.getLogger().debug("DriverInfo.json file doesn't, hence creating the json file with default template.");
		...
		if(!currentVersion.equals(laterVersion)) {
			...
			DownloadWebDrivers.logAccess.getLogger().debug("Latest webdriver is not availble for " + browserName + " on the local machine. Downloading latest version # " + laterVersion);
			String originalZipPath = System.getenv("temp") + browserName +"_"+ dateTimeUtil.getCurrentDateTime() + ".zip";
			...
			DownloadWebDrivers.logAccess.getLogger().debug("Successfully downloaded driver for " + browserName.toUpperCase());
			...
			DownloadWebDrivers.logAccess.getLogger().debug("Using latest webdriver for " + browserName + " browser.\nLatest Version : " + laterVersion );
		}
